create table if not exists public.title_get_clicks (
    title_id uuid not null references public.titles(id) on delete cascade,
    viewer_hash text not null,
    viewer_source text not null check (viewer_source in ('ip_address', 'visitor_id')),
...
create index if not exists idx_title_get_clicks_title_id
    on public.title_get_clicks(title_id);

create index if not exists idx_title_get_clicks_last_clicked_at
    on public.title_get_clicks(last_clicked_at desc);
...
alter table public.title_get_clicks enable row level security;
